...
import { Page, LikeResponse, PostRequest, ImageFile } from '@/types/community/common';
...
// 등록
export const createTipPost = (post: PostRequest, file?: ImageFile) => {
...
// 수정
export const updateTipPost = (postId: number, post: PostRequest, file?: ImageFile, deleted = false) => {
    const formData = new FormData();
    formData.append('post', JSON.stringify(post));
    if (file) formData.append('file', file as any);
