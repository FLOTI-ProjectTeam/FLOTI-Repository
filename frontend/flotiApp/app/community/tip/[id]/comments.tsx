...
export default function CommentListScreen() {
  ...
  const [newComment, setNewComment] = useState(''); // 입력창 상태
  const [replyTo, setReplyTo] = useState<ReplyTo | null>(null);
  const [confirmVisible, setConfirmVisible] = useState(false);
  const [targetComment, setTargetComment] = useState<CommentResponse | null>(null);
  ...
  const handleSend = (replyTo?: ReplyTo | null) => {
    ...
    const newItem: CommentResponse = {
      id: Date.now(),
      postId: Number(id),
      parentId: replyTo?.id || null, // 답글이면 parentId 기록
      author: {
        id: 1,
        nickname: "Alice",
